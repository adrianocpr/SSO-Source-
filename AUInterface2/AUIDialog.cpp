...
void AUIDialog::Show(bool bShow, bool bModal, bool bActive)
{
	...
	else
	{
		...
		if( pos )
		{
			...
			if (m_pAUIManager->GetActiveDialog() == this)
			{
				bool bring_top = false;
				if( m_pAUIManager->m_DlgZOrder.GetCount() > 0 )
				{
					ALISTPOSITION pos = m_pAUIManager->m_DlgZOrder.GetHeadPosition();
					while( pos )
					{
						PAUIDIALOG pDlg = m_pAUIManager->m_DlgZOrder.GetNext(pos);
						if( pDlg && pDlg->IsShow() && NULL == pDlg->GetParentDlgControl())
						{
							m_pAUIManager->BringWindowToTop(pDlg);
							bring_top = true;
							break;
				...
				if (!bring_top)
				{
					if( m_pAUIManager->m_DlgZOrderBack.GetCount() > 0 )
					{
						ALISTPOSITION pos = m_pAUIManager->m_DlgZOrderBack.GetHeadPosition();
						while( pos )
						{
							PAUIDIALOG pDlg = m_pAUIManager->m_DlgZOrderBack.GetNext(pos);
							if( pDlg && pDlg->IsShow() && NULL == pDlg->GetParentDlgControl() && pDlg->IsCanEscClose())
							{
								m_pAUIManager->BringWindowToTop(pDlg);								
								break;
							}
