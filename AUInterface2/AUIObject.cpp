...
void AUIObject::Update()
{
	if(m_pChangeMan)
	{
		m_pChangeMan->Update(this,GetSubject());
	}
}
...
void AUIObject::SetChangeMan(AUIChangeManager* pChangeMan)
{
	if (m_pChangeMan)
	{
		delete m_pChangeMan;
	}
	m_pChangeMan = pChangeMan;
}

void AUIObject::SetDataPtr(AUISafeData* pData,const AString& strKey)
{
	if (pData == NULL)
	{
		Detach();
		m_strSafeDataKey.Empty();
	} 
	else
	{
		AttachTo(pData);
		m_strSafeDataKey = strKey;
	}	
}

AUISafeData* AUIObject::GetDataPtr(const AString& strKey)
{
	if (strKey == m_strSafeDataKey)
	{
		return (AUISafeData*)(GetSubject());
	}
	return NULL;
}
...
void AUIObject::SetRenderHintCallback(LPFRENDERHINTCALLBACK pCallback, DWORD param1, DWORD param2, DWORD param3, DWORD param4)
{
	m_dwParamCallback1 = param1;
	m_dwParamCallback2 = param2;
	m_dwParamCallback3 = param3;
	m_dwParamCallback4 = param4;
...
bool AUIObject::ExecuteRenderHintCallback(POINT ptCursor)
{
	...
	return (*m_pRenderHintCallback)(ptCursor, m_dwParamCallback1, m_dwParamCallback2, m_dwParamCallback3, m_dwParamCallback4);
}
