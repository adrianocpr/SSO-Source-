...
static const DWORD dwCHFileId = 0xffffffff;
...
{
	DWORD dwWrite, dwLen, dwDWSize = sizeof(DWORD);
	af.Write((LPVOID)&dwCHFileId, dwDWSize, &dwLen);
	af.Write((LPVOID)&dwVersion, dwDWSize, &dwLen);
	dwWrite = (DWORD)m_genType;
	af.Write((LPVOID)&dwWrite, dwDWSize, &dwLen);
	af.Write((LPVOID)m_szModelFile, sizeof(m_szModelFile), &dwLen);
	if (m_genType == ECM)
	{		
		af.Write((LPVOID)&m_nFrame, sizeof(int), &dwLen);
	}
