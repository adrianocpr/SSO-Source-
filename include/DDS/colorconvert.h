...
inline void ColorToVector(rgba_t color, float & r, float & g, float & b)
{
    ...
    r = (float)((float)(color.r) / 127.5 - 1.0);
    g = (float)((float)(color.g) / 127.5 - 1.0);
    b = (float)((float)(color.b) / 127.5 - 1.0);
}
...
inline void RGBAToAlphaAndVector(const rgba_t & inColor, unsigned long& theHeight, fpPixel& outVector)
{
    ...
    outVector.x = (double)((double)inColor.r / 127.5 - 1.0);
    outVector.y = (double)((double)inColor.g / 127.5 - 1.0);
    outVector.z = (double)((double)inColor.b / 127.5 - 1.0);
                          ...
                          rgba_t & outColor )
{
    ...
    outColor.a = (unsigned char)theHeight;
    outColor.r = red;
    outColor.g = green;
    outColor.b = blue;
...
void VectorToColor(float r, float g, float b, rgba_t & color)
{
    ...
    int ir = static_cast<int>(((r + 1.0) * 127.5) + 0.5);
    int ig = static_cast<int>(((g + 1.0) * 127.5) + 0.5);
    int ib = static_cast<int>(((b + 1.0) * 127.5) + 0.5);
...
inline void AlphaAndVectorToARGB( const unsigned long & theHeight, 
                                 const fpPixel & inVector,rgba_t * outColor )
{
	int red   = static_cast<int>((( inVector.x + 1.0f ) * 127.5f ) + 0.5 );
	int green = static_cast<int>((( inVector.y + 1.0f ) * 127.5f ) + 0.5 );
	int blue  = static_cast<int>((( inVector.z + 1.0f ) * 127.5f ) + 0.5 );
