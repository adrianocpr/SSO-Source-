...
bool CECSceneLights::Load(const char * szLightFile)
{
	...
	{
		a_LogOutput(1, _EA("CECSceneLights::Load(), failed to open file %s!"), szLightFile);
		return false;
	}
	...
	{
		a_LogOutput(1, _EA("CECSceneLights::Load(), read file failed!"));
		return false;
	}

	if( dwVersion != 0x80000001 )
	{
		a_LogOutput(1, _EA("CECSceneLights::Load(), wrong version!"));
		return false;
	}
	...
	if (nNumLights)
    {
	    ...
	    {
		    a_LogOutput(1, _EA("CECSceneLights::Load(), read file part 2 failed!"));
