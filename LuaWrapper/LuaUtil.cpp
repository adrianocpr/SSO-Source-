...
void ReportError(lua_State * L, const char * szSrc, const char * szBufName, int line, const char * szDesc)
{
	...
	if (L)
	{
		...
		if (CLuaStateMan::GetInstance().GetState(L) && CLuaStateMan::GetInstance().GetState(L)->IsDebugEnabled()) 
		{
			CLuaStateMan::GetInstance().GetState(L)->GetDebug()->ReportError(szBufName, line, szDesc);
		}
