...
{
	if(!CLuaStateMan::GetInstance().GetState(L) || !CLuaStateMan::GetInstance().GetState(L)->GetDebug()->IsInDebug())
		return;
 	switch(ar->event)
 	{
...
//	CLuaStateMan::GetInstance().GetState(L)->GetDebug()->GetDebugger()->		// can't get debugger from m_pDebugger
