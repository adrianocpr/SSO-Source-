...
{
	AWString bridge;
	int codePage = CLuaStateMan::GetInstance().GetCodePage();
...
///////////////////////////////////////////////////////////////////////////
//	
//	Implementation of CScriptContext
//	
///////////////////////////////////////////////////////////////////////////

std::map<DWORD, std::list<CScriptCallContext::_Data> > CScriptCallContext::s_threadMap;

class _CriticalSectionWrapper
{
public:
	_CriticalSectionWrapper()
	{
...
static _CriticalSectionWrapper l_script_context_cs;
__declspec(thread) static int _tls_script_context_depth = 0;

CScriptCallContext::CScriptCallContext(const char* file /* = NULL */, int line /* = 0 */)
:	m_pData(NULL),
	m_fileName(file),
	m_line(line)
{
	_tls_script_context_depth++;
	bool bSuccess = init_data();
	assert(bSuccess);
}

CScriptCallContext::~CScriptCallContext()
{
	args().clear();
	results().clear();
	m_pData = NULL;
...
bool CScriptCallContext::init_data()
{
	assert(_tls_script_context_depth > 0);
	DWORD tid = GetCurrentThreadId();

	if(_tls_script_context_depth <= 0)
	{
		AString msg;
		msg.Format("%s:%d CScriptCallContext::init_data() failed, thread %d", 
			m_fileName.IsEmpty() ? "[Unknown]" : m_fileName,
			m_line,
			tid);
		a_LogOutput(1, msg);
		return false;
	...
	std::list<_Data>* pList = NULL;
	{
		ACriticalSectionAutoLock _auto_lock(&l_script_context_cs.m_cs);

		DWORD tid = GetCurrentThreadId();
		std::list<_Data>& list = s_threadMap[tid];
		pList = &list;
	}
	if(!pList) return false;

	if(_tls_script_context_depth > DEFAULT_DEPTH)
	{
		AString msg;
		msg.Format("%s:%d Warning: script_context_depth is %d, thread %d", 
			m_fileName.IsEmpty() ? "[Unknown]" : m_fileName,
			m_line,
			_tls_script_context_depth,
			tid);
	...
	if(_tls_script_context_depth > (int)pList->size())
	{
		// increase size only, do not decrease.
		pList->resize(_tls_script_context_depth);
	}

	std::list<_Data>::iterator it = pList->begin();
	std::advance(it, _tls_script_context_depth - 1);
	m_pData = it != pList->end() ? &(*it) : NULL;
	assert(m_pData);
	if(!m_pData) return false;

	args().reserve(DEFAULT_ARGS_CAPACITY);
	results().reserve(DEFAULT_ARGS_CAPACITY);
	args().clear();
	results().clear();
...
{
	return Call(L, szTblName, szMethod, &args, &results);	
}

bool Call( lua_State*L, const char * szTblName, const char * szMethod, CScriptCallContext& context )
{
	return Call(L, szTblName, szMethod, context.args(), context.results());
}

bool Call( lua_State*L, const char * szTblName, const char * szMethod, const abase::vector<CScriptValue>* args /*= NULL*/, abase::vector<CScriptValue>* results /*= NULL*/ )
{
	int n = lua_gettop(L); //save the stack pos
	if(args)
	{
