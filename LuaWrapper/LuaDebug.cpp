...
void CLuaDebug::SendBuff() {
	CLuaState::ScriptMap *scripts =  m_pState->GetAllScripts();
	for (CLuaState::ScriptMap::iterator it = scripts->begin(); it != scripts->end(); ++it)
	{
	...
	for (abase::hash_map<AString, bool>::iterator it = m_buffMap.begin(); it != m_buffMap.end(); ++it)
	{
		senbufMsg.Release();
		if(it->second != true) {
#ifdef _UNICODE
			AWString _buffname;
			CScriptString temp;
			temp.SetAString(it->first);
			temp.RetrieveAWString(_buffname);
#else
#define _buffname it->first
#endif
