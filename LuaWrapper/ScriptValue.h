...
#define DEFINE_SCRIPT_CALL_CONTEXT(ctx_name, args_name, results_name)\
	CScriptCallContext ctx_name(__FILE__, __LINE__);\
	abase::vector<CScriptValue>& args_name = ctx_name.args();\
	abase::vector<CScriptValue>& results_name = ctx_name.results();
...
public:
	CScriptCallContext(const char* file = NULL, int line = 0);
	virtual ~CScriptCallContext();

	abase::vector<CScriptValue>& args() { return m_pData->args; }
	abase::vector<CScriptValue>& results() { return m_pData->results; }
...
private:
	bool init_data();
	_Data* m_pData;
	static std::map<DWORD, std::list<_Data> > s_threadMap;
...
namespace LuaBind
{
	...
	bool Call(lua_State*L, const char * szTblName, const char * szMethod, 
		const abase::vector<CScriptValue>* args = NULL, abase::vector<CScriptValue>* results = NULL);
