...
void CDebugger::RunToCursor(const _TCHAR* szName, int nLine)
{
	...
	m_nLine = nLine;
	if (szName) {
		memcpy(m_szBuffName, szName, sizeof(m_szBuffName));
	}
}
...
#define _szFile szFile
	...
	{
		CLuaState::AutoLock lock(m_pLuaDebug->GetState());
...
void CDebugger::DebugBreak(const _TCHAR *szFile, int nLine)
{
	...
	memcpy(ldmBreak.callStack, c_src, ldmBreak.stackLen*sizeof(c_src[0]));
...
void CDebugger::GotoStackTraceLevel(int nLevel)
{
	...
	memcpy(showBufferLine.name, m_buffs[nLevel], sizeof(showBufferLine.name));
