...
void A3DAutoButterfly::Update (DWORD dwDeltaTime, ActionList_e action /*= ACTION_FIRST*/)
{
	...
	if(!m_bDoingAction)
	{
		if( m_bUseECModel)
		{
			...
			{
				m_pECModel->PlayActionByName(_EA("·ÉÐÐ"), 1.0f);
			}
		}
		else
		{
			if( action == ACTION_FIRST)
			{
				m_pSkinModel->PlayActionByNameDC(_EA("·ÉÐÐ"), -1);
			}
		}
		m_bDoingAction = true;
	}
...
void A3DAutoButterfly::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater2 * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoButterfly::ClampAboveTerrainWater (A3DVECTOR3 pos, A3DTerrainWater2* pTerrainWater)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
