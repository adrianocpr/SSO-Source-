...
void A3DAutoCrow::Update (DWORD dwDeltaTime, ActionList_e action /*= ACTION_FIRST*/)
{
	...
	if(m_CurAction != action)
	{
		if( m_bUseECModel)
		{
			...
			{
				m_pECModel->PlayActionByName(_EA("øÏÀŸ∑…––"), 1.0);
			}
			else if( action == ACTION_SECOND)
			{
				m_pECModel->PlayActionByName(_EA("–›œ–"), 1.0);
			}
...
//			if( action == ACTION_FIRST)
//			{
//				m_pSkinModel->PlayActionByName(_EA("øÏÀŸ∑…––"), 1.0f);
//			}
//			else if( action == ACTION_SECOND)
//			{
//				m_pSkinModel->PlayActionByName(_EA("–›œ–"), 1.0f);
//			}

			m_pSkinModel->PlayActionByNameDC(_EA("…»∂Ø≥·∞Ú"), -1); //test
...
void A3DAutoCrow::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater2 * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoCrow::ClampAboveTerrainWater (A3DVECTOR3 pos, A3DTerrainWater2* pTerrainWater)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
