...
void A3DAutoEagle::Update (DWORD dwDeltaTime, ActionList_e action /*= ACTION_FIRST*/)
{
	...
	if(!m_bDoingAction)
	{
		if( m_bUseECModel)
		{
			...
			{
				m_pECModel->PlayActionByName(_EA("…»∂Ø≥·∞Ú"), 1.0f);
			}
		}
		else
		{
			if( action == ACTION_FIRST)
			{
				m_pSkinModel->PlayActionByNameDC(_EA("…»∂Ø≥·∞Ú"), -1);
			}
		}
...
void A3DAutoEagle::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater2 * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoEagle::ClampAboveTerrainWater (A3DVECTOR3 pos, A3DTerrainWater2* pTerrainWater)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
