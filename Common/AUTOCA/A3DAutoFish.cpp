...
void A3DAutoFish::Update (DWORD dwDeltaTime, ActionList_e action /*= ACTION_FIRST*/)
{
	...
	if(!m_bDoingAction)
	{
		if( m_bUseECModel)
		{
			...
			{
				m_pECModel->PlayActionByName(_EA("游动"), 1.0f);
			}
		}
		else
		{
			if( action == ACTION_FIRST)
			{
				m_pSkinModel->PlayActionByName(_EA("游动"), 1.0f);
			}
		}
		m_bDoingAction = true;
	}
...
void A3DAutoFish::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater2 * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoFish::ClampUnderTerrainWater (A3DVECTOR3 pos, A3DTerrainWater2* pTerrainWater)
{
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
	A3DVECTOR3 acc = GetAcceleration();
...
void A3DAutoBriskFish::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater2 * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoBriskFish::ClampUnderTerrainWater (A3DVECTOR3 pos, A3DTerrainWater2* pTerrainWater)
{
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
	A3DVECTOR3 acc = GetAcceleration();
...
void A3DAutoDoltishFish::Update (DWORD dwDeltaTime, ActionList_e action /*= ACTION_FIRST*/)
{
	...
	if(!m_bDoingAction)
	{
		...
		else
		{
			if( action == ACTION_FIRST)
			{
				m_pSkinModel->PlayActionByNameDC(_EA("游动"), -1);
			}
		}
...
void A3DAutoDoltishFish::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater2 * pTerrainWater, DWORD dwDeltaTime)
{
	A3DVECTOR3 newpos = PredictPosition(4*dwDeltaTime);		
	if(pTerrain)		ClampAboveTerrain(newpos, pTerrain);
	if(pTerrainWater)	ClampUnderTerrainWater(newpos, pTerrainWater);
...
void A3DAutoDoltishFish::ClampUnderTerrainWater (A3DVECTOR3 pos, A3DTerrainWater2* pTerrainWater)
{
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
	A3DVECTOR3 acc = GetAcceleration();
