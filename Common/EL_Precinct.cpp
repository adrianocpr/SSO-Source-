...
CELPrecinct::CELPrecinct()
{
	...
	for( int i = 0; i < 4; ++i)
	{
		m_RebornPos[i].uFactionMask = 0;
		m_RebornPos[i].nMapId = 0;
		m_RebornPos[i].vPos.x = 0;
		m_RebornPos[i].vPos.y = 0;
		m_RebornPos[i].vPos.z = 0;
	}
...
bool CELPrecinct::Load(AWScriptFile* pFile, int iVersion)
{
	...
	if(iVersion < 11)
	{
		...
		iNumMusic = pFile->GetNextTokenAsInt(false);
		//	Read music interval
	...
	if(iVersion > 8)
		m_iCameraIndex = pFile->GetNextTokenAsInt(false);
	else m_iCameraIndex = 0;

	if(iVersion > 10)
	{
		m_fHeightTop = pFile->GetNextTokenAsFloat(false);
		m_fHeightBottom = pFile->GetNextTokenAsFloat(false);
	}
	...
	if( iVersion < 7)
	{
		pFile->GetNextTokenAsFloat(true);
		pFile->GetNextTokenAsFloat(false);
		pFile->GetNextTokenAsFloat(false);
	}else
	{
		for(int i = 0; i < 4; ++i)
		{
			m_RebornPos[i].uFactionMask = pFile->GetNextTokenAsInt(true);
			m_RebornPos[i].nMapId = pFile->GetNextTokenAsInt(false);
			m_RebornPos[i].vPos.x = pFile->GetNextTokenAsFloat(false);
			m_RebornPos[i].vPos.y = pFile->GetNextTokenAsFloat(false);
			m_RebornPos[i].vPos.z = pFile->GetNextTokenAsFloat(false);
		}
	}
	...
	for (i=0; i < iNumMark; i++)
	{
		...
		if( iVersion > 7) pMark->uType = pFile->GetNextTokenAsInt(false);
		else pMark->uType = 0;
	...
	//	Read sound file
	pFile->GetNextToken(true);
	m_strSound = AString(WC2AS(pFile->m_szToken));
	...
	pFile->GetNextToken(true);
	m_strSound_n = AString(WC2AS(pFile->m_szToken));
	if(iVersion > 10)
	{
		pFile->GetNextToken(true);
		m_strMusic = AString(WC2AS(pFile->m_szToken));
		pFile->GetNextToken(true);
		m_strMusic_n = AString(WC2AS(pFile->m_szToken));
	}
	//	Build bound box
	BuildBoundBox();
...
bool CELPrecinct::Load(FILE* fp, int iVersion)
{
	...
	{
		fread(m_RebornPos, sizeof (REBORN_PT)*4, 1, fp);
	}
...
CELPrecinct* CELPrecinctSet::IsPointIn(float x, float z, int idSrcInst/*=-1*/)
{
	...
	for (int i=0; i < (int)m_aPrecincts.size(); i++)
	{
		...
		if ((idSrcInst >= 0 && p->GetSrcInstanceID() != idSrcInst) || !p->IsPointIn(x, z))
			continue;
...
CELPrecinct* CELPrecinctSet::GetPrecinctByID(const unsigned int id)
{
	for (int i=0; i < (int)m_aPrecincts.size(); i++)
	{
		CELPrecinct* p = m_aPrecincts[i];
...
bool CELPrecinctSet::Load(const char* szFileName)
{
	...
	//	Get version
	if (!File.MatchToken((wchar_t*)_EW("version"), false))
		return false;
