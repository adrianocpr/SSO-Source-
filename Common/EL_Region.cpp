...
bool CELRegion::Load(AWScriptFile* pFile, int iVersion)
{
	if (iVersion >= 2)
	{
		if (!pFile->MatchToken((wchar_t*)_EW("[region]"), true))
			return false;
	}
	...
	//  
	m_nControllerID = pFile->GetNextTokenAsInt(true); //  new for ver.7
	//	Read point number
...
bool CELRegion::Load(FILE* fp, int iVersion)
{
	...
	//  Read type and controller.
	fread(&m_iType, sizeof(int), 1, fp); // new for ver.7
	fread(&m_nControllerID, sizeof(int), 1, fp); // new for ver.7
...
bool CELTransportBox::Load(AWScriptFile* pFile, int iVersion)
{
	if (iVersion >= 2)
	{
		if (!pFile->MatchToken((wchar_t*)_EW("[trans]"), true))
			return false;
	}
	...
	if (iVersion >= 6)
		m_iControllerID = pFile->GetNextTokenAsInt(false);
...
bool CELTransportBox::Load(FILE* fp, int iVersion)
{
	...
	if (iVersion >= 6)
		fread(&m_iControllerID, sizeof (int), 1, fp);
...
//	Get the region specified position is in, with priority
CELRegion* CELRegionSet::IsPointInRegion(float x, float z, RegionStateVec& vec)
{
	for (size_t i = 0; i < m_aRegions.size(); i++)
	{
		if (i >= 0 && i < vec.size())
		{
			if (vec.test(i))
	...
	for (size_t i = 0; i < m_aRegions.size(); i++)
	{
		...
		if (i >= 0 && i < vec.size())
		{
			bPriority = vec.test(i);
		}
		if (bPriority) continue;
...
//	Get the Freely PK region specified position is in
CELRegion* CELRegionSet::IsPointInPKFreeRegion(float x, float z)
{
	for (int i=0; i < (int)m_aPKFreeRegions.size(); i++)
	{
		CELRegion* p = m_aPKFreeRegions[i];
		if (p->IsPointIn(x, z))
			return p;
	}
...
CELTransportBox* CELRegionSet::IsPointInTransport(float x, float y, float z, int idSrcInst)
{
	for (int i=0; i < (int)m_aTransBoxes.size(); i++)
	{
		...
		if ( (p->GetSrcInstanceID() == -1 || p->GetSrcInstanceID() == idSrcInst) && p->IsPointIn(x, y, z))
			return p;
	}
...
bool CELRegionSet::Load(const char* szFileName)
{
	...
	{
		if (iVersion == 1 || !wcsicmp(File.m_szToken, _EW("[region]")))
		{
			...
			else if (pRegion->GetType() == CELRegion::REGION_PK_FREE)
				m_aPKFreeRegions.push_back(pRegion);
		}
		else if (!wcsicmp(File.m_szToken, _EW("[trans]")))
		{
...
bool CELRegionSet::Load(const char* szFileName)
{
	FILE* fp = fopen(szFileName, _EA("rb"));
	if (!fp)
		return false;
