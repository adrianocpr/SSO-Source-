...
#define TRIANGLE_ARRAY_NUM 64

class CTriangleGrid;
class CTriangleCell;
struct TManTriangle;
class CTManTriangleArray;
...
struct TManTriangle
{
	...
	DWORD GetRef()const { return dwRef; }
	void  AddRef(){ ++dwRef; }
...
public:
	...
	TManTriangle* CreateTriangle(int& indexTriangle);
	void		  ReleaseTriangle(const int indexTriangle);
	TManTriangle* GetTriangle(const int indexTriangle);
...
private:
	TManTriangle*		m_pTriangleArray;
	abase::vector<int>	m_vecFreeIndex;
};

class CTManTrianglePoolEx
{
public:
	CTManTrianglePoolEx();
	~CTManTrianglePoolEx();
	...
	TManTriangle* CreateTriangle(int& id);
	void		  ReleaseTriangle(const int id);
	TManTriangle* GetTriangle(const int id);

	void Tick(const DWORD dwDeltaTime);
	
protected:
	CTManTriangleArray m_array[TRIANGLE_ARRAY_NUM];
...
private:
	...
	CTManTrianglePoolEx   m_trianglePool;
	...
	inline	bool   _AddTriangle(TManTriangle * pTri, int idTriangle);
