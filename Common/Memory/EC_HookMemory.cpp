...
#error "Not supported on this architecture."
	...
	sf.AddrPC.Offset    = (unsigned long)programcounter;
	sf.AddrFrame.Offset = (unsigned long)framepointer;
	...
	for( ; ; )
	{		
		if( addr_count >= (client_memory_operate_info::ADDRESS_NUM+2) || !StackWalk64(architecture, GetCurrentProcess(), GetCurrentThread(), &sf, &c, NULL, SymFunctionTableAccess64, SymGetModuleBase64, NULL ) )
		{
			break;
		}
