...
#ifdef BMAN_DEBUG
	char msg[100];
	sprintf_s(msg, sizeof(msg) / sizeof(char), _EA("remove provider\n"));
...
#ifdef BMAN_DEBUG
	char msg[100];
	sprintf_s(msg, sizeof(msg) / sizeof(char), _EA("add provider \n"));
	OUTPUT_DEBUG_INFO(msg);
#endif
...
bool CBrushGrid::RemoveProvider(CBrushProvider * pProvider)
{
	...
	for (i = 0; i < pProvider->GetBrushCount(); ++i)
	{
		...
		else
		{
			...
			{
				assert(0&& _EA("not in the out of range brush!"));
				continue;
			}
	...
	for (iz = iz0; iz < iz0 + m_iH; ++iz)
	{
		for (ix = ix0; ix < ix0 + m_iW; ++ix)
		{
			...
			{//need replace 
				...
				assert(pCell != NULL && _EA("replace cell not exist!"));
				if (pCell)
...
void CBrushGrid::Build(const A3DVECTOR3& vCenter, bool bForce)
{
...
#ifdef BMAN_DEBUG
	char msg[200];
	sprintf_s(msg, sizeof(msg) / sizeof(char), _EA("build center %f %f %f "), vCenter.x, vCenter.y, vCenter.z);
	OUTPUT_DEBUG_INFO(msg);
#endif
#if BMAN_VERBOSE_STAT
	DWORD tStart;
...
#ifdef BMAN_DEBUG
	sprintf_s(msg, sizeof(msg) / sizeof(char), _EA(" update center "));
	OUTPUT_DEBUG_INFO(msg);
#endif
	int i, j;
	...
	m_UnOrganizedProvider.clear();
#ifdef BMAN_DEBUG
	sprintf_s(msg, sizeof(msg) / sizeof(char), _EA(" unorg provider "));
	OUTPUT_DEBUG_INFO(msg);
#endif
	if (bCenterChange)
	{
...
#endif	
#ifdef BMAN_DEBUG
	sprintf_s(msg, sizeof(msg) / sizeof(char), _EA(" build ok\n"));
	OUTPUT_DEBUG_INFO(msg);
#endif
}
...
bool CBrushGrid::Trace(BrushTraceInfo * pInfo, bool bCheckFlag /* = true */)
{
	...
	ECPerformanceWrapper ecpw(g_pGame->GetECPerformance(), ECPerformance::PERF_SP_BRUSHTRACE);
#endif
...
// 		}
		...
		for (j = 0; j < (int)pCell->m_pBrushes.size(); ++j)
		{
			...
			A3DAABB aabb = pBrush->GetAABB();
			if (pInfo->bIsPoint 
				&& !CLS_RayToAABB3(pInfo->vStart, pInfo->vDelta, aabb.Mins, aabb.Maxs, vDummyPoint, &fDummyFraction, vDummyNormal ) )
			{
				continue;
			}
			if (!pInfo->bIsPoint 
				&& !CLS_AABBAABBOverlap(aabb.Center, aabb.Extents, pInfo->BoundAABB.Center, pInfo->BoundAABB.Extents) ) 
			{
				continue;
			}
