...
bool CELForest::Init(A3DDevice * pA3DDevice, A3DLight * pDirLight, ALog * pLogFile)
{
	...
	CSpeedTreeRT::Authorize(_EA("RT-0637-9ea30d39f89d"));
	...
	{
		m_pLogFile->Log(_EA("CELForest::Init(), failed to call CreateStream()!"));
		return false;
	}
...
#ifdef TREE_BUMP_ENABLE
	m_pBranchPixelShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader(_EA("shaders\\ps\\treebranch.txt"), false);
	...
	m_pBranchVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader(_EA("shaders\\vs\\treebranchvs.txt"), false, dwVSDecl);
#endif
	return true;
}
...
bool CELForest::CreateStream(int nMaxBlades)
{
	...
	{
		m_pLogFile->Log(_EA("CELForest::CreateStream(), failed to initialize stream!"));
		return false;
	}
...
bool CELForest::LoadTypesFromConfigFile(const char* szConfigFile)
{
	...
	{
		m_pLogFile->Log(_EA("CELForest::Init(), Can not open config file!"));
		return false;
	}
	...
	int		nNumTreeType;
	sscanf(szLine, _EA("%d"), &nNumTreeType);
	...
	for(int i=0; i<nNumTreeType; i++)
	{
		...
		configFile.ReadLine(szLine, 256, &dwReadLen);
		sscanf(szLine, _EA("%d"), &dwTypeID);

		configFile.ReadLine(szLine, 256, &dwReadLen);
		CELTree * pNewTree = new CELTree();
		if( !pNewTree->Init(m_pA3DDevice, this, dwTypeID, szLine, _EA("trees\\Sample_Trees\\CompositeMap.dds"), m_pLogFile) )
		{
			m_pLogFile->Log(_EA("CELForest::Init(), failed to load tree type [%s]"), szLine);
			return false;
		}
...
{	
	...
	{
		m_pLogFile->Log(_EA("CELForest::Load(), failed to open file [%s]"), szForestFile);
		return true;
	}
	...
	{
		if( !LoadTypesFromConfigFile(_EA("trees\\trees.cfg")) )
		{
			m_pLogFile->Log(_EA("CELForest::Load(), failed to load tree types from config file!"));
			return false;
		}
		...
		{
			m_pLogFile->Log(_EA("CELForest::Load(), tree type library has been modified, so all tree data can't be loaded!"));
			return false;
		}
	}
	else
	{
		...
		for(i=0; i<nNumTypes; i++)
		{
			...
			{
				m_pLogFile->Log(_EA("CELForest::Load(), failed to init tree type [%s]"), szTreeFile);
				return false;
			}
	...
	for(i=0; i<nNumTypes; i++)
	{
		...
		{
			m_pLogFile->Log(_EA("CELForest::Load(), failed to call CELTree::Load()!"));
			return false;
		}
	}
...
READFAIL:
	m_pLogFile->Log(_EA("CELForest::Load(), Read from file failed!"));
	return false;
}
...
bool CELForest::Save(const char * szForestFile)
{
	...
	{
		m_pLogFile->Log(_EA("CELForest::Save(), failed to create file [%s]"), szForestFile);
		return false;
	}
	...
	for(i=0; i<nNumTypes; i++)
	{
		...
		{
			m_pLogFile->Log(_EA("CELForest::Save(), failed to call CELTree::Save()!"));
			return false;
		}
	}
...
WRITEFAIL:
	m_pLogFile->Log(_EA("CELForest::Save(), Write to the file failed!"));
	return false;
}
...
CELTree* CELForest::AddTreeType(DWORD dwTypeID, const char * szTreeFile, const char * szCompositeMap)
{
	...
	if( !pNewTree->Init(m_pA3DDevice, this, dwTypeID, szTreeFile, szCompositeMap, m_pLogFile) )
	{
		m_pLogFile->Log(_EA("CELForest::AddTreeType(), failed to load tree type [%s]"), szTreeFile);
		return NULL;
	}
