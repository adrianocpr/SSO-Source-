...
// turn off the warning in hashtab.h : check operator–>( ) return.  By kuiwu. [15/3/2006]
...
const static int TRIANGLE_ARRAY_SIZE = (512*1024)/sizeof(TManTriangle); // 512K memory(This size is good for windows memory management)
const static int TRIANGLE_MAN_TICK_INTERVAL = 60*1000;//60 seconds

static int inline TriangleIndexToID(const int indexArray, const int indexTriangle)
{
	ASSERT(indexTriangle<0xFFFFFF);
	int id = ((indexArray&0xFF)<<24) | ((indexTriangle+1)&0xFFFFFF);
	return id;
}

static void inline TriangleIDToIndex(int& indexArray, int& indexTriangle, const int id)
{
	ASSERT(id);
	indexArray = (id&0xFF000000)>>24 ;
	indexTriangle = (id&0xFFFFFF)-1;
	ASSERT(indexTriangle<0xFFFFFF);
}
...
void CECTriangleMan::Tick(const DWORD dwDeltaTime)
{ 
	if(m_pGrid)
	{ 
		m_pGrid->Tick(dwDeltaTime); 
	}
}
...
bool CTriangleGrid::RemoveProvider(CELBrushBuilding * pProvider)
{
	...
	for (it = m_CellTbl.begin(); it != m_CellTbl.end(); ++it)
	{
		...
		leftIndex.clear();
		leftIndex.reserve(pCell->m_Triangles.size());
		for (j = 0; j < (int)pCell->m_Triangles.size(); ++j)
		{
			const TManTriangle * pTri = m_trianglePool.GetTriangle(pCell->m_Triangles[j]);
			assert(pTri);
			if(pTri && pTri->pProvider == pProvider)
			{
				m_trianglePool.ReleaseTriangle(pCell->m_Triangles[j]);
			}
			else
			{
...
int CTriangleGrid::_AddProvider(CELBrushBuilding * pProvider)
{
	...
	for (int j = 0; j < pLitModel->GetNumMeshes(); ++j)
	{
		...
		{
			int idTriangle = 0;
			TManTriangle* pTri = m_trianglePool.CreateTriangle(idTriangle);
			if(!pTri)
			{
				ASSERT(0);
				return nAdd;
			...
			_AddTriangle(pTri, idTriangle);	
			if (!pTri->GetRef())
			{
				m_trianglePool.ReleaseTriangle(idTriangle);
			}
			++nAdd;
		}
	}
...
void CTriangleGrid::Build(const A3DVECTOR3& vCenter, DWORD dwFlag)
{
	...
	if (bCenterChange)
	{
		...
		A3DAABB  gridAABB;
		gridAABB.Center = vCenter;//todo: this should be cell center, not player pos. [liudong 2011/4/11 14:14]
...
void CTriangleGrid::BuildGradually(const A3DVECTOR3& vCenter, DWORD dwFlag /* = TMAN_BUILD_DEFAULT */)
{
	...
	CELBrushBuilding  * pProvider;
	vector<int> leftIndex;
	leftIndex.reserve(m_UnOrganizedProvider.size());
	A3DAABB prvAABB;
	int nAdd = 0;
	for (int i = 0; i < (int)m_UnOrganizedProvider.size(); ++i )
	{
...
bool CTriangleGrid::_AddTriangle(TManTriangle * pTri,  int idTriangle)
{
	...
	for (zz = zMin; zz <= zMax; ++zz)
	{
		for (xx = xMin; xx <= xMax; ++xx)
		{
			...
			pCell->m_Triangles.push_back(idTriangle);
...
void CTriangleGrid::TestOverlapNF(TMan_Overlap_Info& tInfo)
{
	...
	for (iz = iz0; iz < iz0 + nHalf*2; ++iz)
	{
		for (ix = ix0; ix < ix0 + nHalf*2; ++ix)
		{
			...
			for (int j = 0; j < (int)pCell->m_Triangles.size(); ++j)
			{
				TManTriangle * pTri = m_trianglePool.GetTriangle(pCell->m_Triangles[j]);
				ASSERT(pTri);
				if (pTri && pTri->dwCheckStamp != m_dwTestStamp)
				{
#ifdef TMAN_VERBOSE_STAT
...
void CTriangleGrid::Tick(const DWORD dwDeltaTime)
{
	m_trianglePool.Tick(dwDeltaTime);
}
...
//	if ((DotProduct(tri.vVert[0] - tInfo.pCamera->GetPos(), tri.vNormal) > 0.0f) || //进行视线剔除，会导致视野之外的很多物品看不见影子，不利于上层对影子数据做缓存。
	if ((DotProduct(tInfo.vDelta, tri.vNormal) > 0.0f))
	{
		return false;
	}
	...
	//todo: 下面这几步可能不需要，直接用原始三角形就行，三角形多了反而费 [liudong 2012/2/8/ 11:27]
...
CTManTriangleArray::CTManTriangleArray()
{
...
void CTManTriangleArray::Release()
{
	if (m_pTriangleArray)
	{
...
TManTriangle* CTManTriangleArray::CreateTriangle(int& indexTriangle)
{
	indexTriangle = 0;
	if (!m_pTriangleArray)
	{
		m_pTriangleArray = new TManTriangle[TRIANGLE_ARRAY_SIZE];
		m_vecFreeIndex.clear();
		m_vecFreeIndex.reserve(TRIANGLE_ARRAY_SIZE);
		for (int i=(TRIANGLE_ARRAY_SIZE-1); i>=0; --i)
		{
			m_vecFreeIndex.push_back(i);
		}
	}
	if (m_vecFreeIndex.size())
	{
		int index = m_vecFreeIndex.back();
		m_vecFreeIndex.pop_back();
		if (index>=0 && index<TRIANGLE_ARRAY_SIZE)
		{
			TManTriangle* p = m_pTriangleArray+index;
			if (0==p->dwRef)
			{
				indexTriangle = index;
				return p;
...
void CTManTriangleArray::ReleaseTriangle(const int indexTriangle)
{
	if (!m_pTriangleArray || indexTriangle<0 || indexTriangle>=TRIANGLE_ARRAY_SIZE)
	{
		ASSERT(0);
		return;
	}
	TManTriangle* p = m_pTriangleArray + indexTriangle;
	if (p->dwRef>0)
	{
		--p->dwRef;
	}
	if (0==p->dwRef)
	{
		m_vecFreeIndex.push_back(indexTriangle);
	}
}


TManTriangle* CTManTriangleArray::GetTriangle(const int indexTriangle)
{
	if (!m_pTriangleArray || indexTriangle<0 || indexTriangle>=TRIANGLE_ARRAY_SIZE)
	{
		ASSERT(0);
...
bool CTManTriangleArray::Compact()
{
	if (m_vecFreeIndex.size()==TRIANGLE_ARRAY_SIZE)
	{
		Release();
		return true;
	}
...
#ifdef _ALPHA_BUILD
void CTManTriangleArray::Check()
{
	if (!m_pTriangleArray)
	{
		return;
	}
	abase::hash_map<int, bool> hashFree;
	for (size_t i=0; i<m_vecFreeIndex.size(); ++i)
	{
		hashFree[m_vecFreeIndex[i]] = true;
	}
	for (size_t i=0; i<TRIANGLE_ARRAY_SIZE; ++i)
	{
		TManTriangle* p = m_pTriangleArray+i;
		if (hashFree.find(i)!=hashFree.end())
		{
			//free
			ASSERT(p->GetRef()==0);
		}else
...
CTManTrianglePoolEx::CTManTrianglePoolEx()
{
	ASSERT(TRIANGLE_ARRAY_NUM<=256);
	ASSERT(TRIANGLE_ARRAY_SIZE<0xFFFFFF);
	m_dwElapsedTime = 0;
}
...
void CTManTrianglePoolEx::Release()
{
	for (unsigned int i=0; i<TRIANGLE_ARRAY_NUM; ++i)
	{
		m_array[i].Release();
	}
}
...
TManTriangle* CTManTrianglePoolEx::CreateTriangle(int& id)
{
	id = 0;
	for (int i=0; i<TRIANGLE_ARRAY_NUM; ++i)
	{
		int indexTriangle = 0;
		TManTriangle* p = m_array[i].CreateTriangle(indexTriangle);
		if (p)
		{
			id = TriangleIndexToID(i, indexTriangle);
			return p;
		}
	}
	return NULL;
...
void CTManTrianglePoolEx::ReleaseTriangle(const int id)
{
	if (!id)
	{
		ASSERT(0);
	...
	int indexArray;
	int indexTriangle;
	TriangleIDToIndex(indexArray, indexTriangle, id);
	if (indexArray<0 || indexArray>=TRIANGLE_ARRAY_NUM)
	{
		ASSERT(0);
		return;
	}
	m_array[indexArray].ReleaseTriangle(indexTriangle);
}

TManTriangle* CTManTrianglePoolEx::GetTriangle(const int id)
{
	if (!id)
	{
		return NULL;
	...
	return m_array[indexArray].GetTriangle(indexTriangle);
}

void CTManTrianglePoolEx::Tick(const DWORD dwDeltaTime)
{
	bool bReleased = false;
	m_dwElapsedTime += dwDeltaTime;
	if (m_dwElapsedTime>TRIANGLE_MAN_TICK_INTERVAL)
	{
		m_dwElapsedTime = 0;
		for (unsigned int i=0; i<TRIANGLE_ARRAY_NUM; ++i)
		{
...
// 			m_array[i].Check();
// #endif
			if(m_array[i].Compact())
				bReleased = true;
		}
	}
	if(bReleased)
		LOG_TIME(L"release triangle pool\n");
}
